...
import {
  ...
  getModelConnections,
  getModelObject,
  getModelObjectDependenciesJson,
  getModelObjectsCsv,
  getModelObjects,
  handleApiError,
  updateModelObject,
...
const GetModelObjectsCsvSchema = z
  .object({
    landscapeId: IcePanelIdSchema,
    response_format: ResponseFormatSchema,
  })
...
const GetModelObjectDependenciesSchema = z
  .object({
    landscapeId: IcePanelIdSchema,
    modelObjectId: IcePanelIdSchema,
    response_format: ResponseFormatSchema,
...
and technologies it uses.`,
  ...
  server.registerTool(
    "icepanel_get_model_objects_csv",
    {
      title: "Get IcePanel Model Objects CSV",
      description: `Export model objects as CSV.`,
      inputSchema: GetModelObjectsCsvSchema,
      annotations: {
        readOnlyHint: true,
        destructiveHint: false,
        idempotentHint: true,
    ...
    async ({ landscapeId, response_format }) => {
      try {
        const csv = await getModelObjectsCsv(landscapeId);
  ...
  server.registerTool(
    "icepanel_get_model_object_dependencies_json",
    {
      title: "Get IcePanel Model Object Dependencies",
      description: `Get dependency data for a model object.`,
      inputSchema: GetModelObjectDependenciesSchema,
      annotations: {
        readOnlyHint: true,
        destructiveHint: false,
        idempotentHint: true,
    ...
    async ({ landscapeId, modelObjectId, response_format }) => {
      try {
        const result = await getModelObjectDependenciesJson(landscapeId, modelObjectId);
        const markdown = `# Model Object Dependencies\n- Model Object ID: ${modelObjectId}`;
        return {
